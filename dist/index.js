...
`;class ta extends tr{static{this.styles=tl}setConfig(t){if(!t.pool)throw Error("pool is required");if(!t.zone_id)throw Error("zone_id is required");this._config=t}set hass(t){this._hass=t;let e=`sensor.sonos_pool_${this._config.pool}`,i=t.states[e];if(!i){this._player=null;return}let s=(i.attributes.assignments||{})[this._config.zone_id];if(!s){this._player=null;return}let r=t.states[s];if(!r){this._player=null;return}let o=r.attributes;this._player={entityId:s,friendlyName:o.friendly_name||s.replace("media_player.",""),state:r.state,title:o.media_title||"",artist:o.media_artist||"",picture:o.entity_picture||"",volume:o.volume_level??0}}async _grab(){let t=await this._hass.connection.sendMessagePromise({type:"call_service",domain:"sonos_pool",service:"grab",service_data:{pool:this._config.pool,zone_id:this._config.zone_id},return_response:!0}),e=t?.response;if(!e?.dante_tx_l||!e?.dante_tx_r)return;let{dante_rx_l:i,dante_rx_r:s}=this._config;i&&this._hass.callService("select","select_option",{entity_id:i,option:e.dante_tx_l}),s&&this._hass.callService("select","select_option",{entity_id:s,option:e.dante_tx_r})}_release(){this._hass.callService("sonos_pool","release",{pool:this._config.pool,zone_id:this._config.zone_id})}_mediaCommand(t){this._player&&this._hass.callService("media_player",t,{entity_id:this._player.entityId})}_setVolume(t){if(!this._player)return;let e=parseFloat(t.target.value);this._hass.callService("media_player","volume_set",{entity_id:this._player.entityId,volume_level:e})}render(){let t=this._config.name||this._config.zone_id;if(!this._player)return L`
        <ha-card>
      ...
      `;let e=this._player,i="playing"===e.state;return L`
      <ha-card>
        <div class="card-header">${t}</div>
        <div class="card-content assigned">
          <div class="player-info">
            ${e.picture?L`<img class="album-art" src="${e.picture}" />`:V}
            <div class="track-info">
              <div class="player-name">${e.friendlyName}</div>
              ${e.title?L`<div class="title">${e.title}</div>`:V}
              ${e.artist?L`<div class="artist">${e.artist}</div>`:V}
            </div>
          </div>
          <div class="controls">
...
`;class tc extends tr{static{this.styles=th}setConfig(t){this._config=t}set hass(t){this._hass=t,this._discoverPools(),this._discoverDanteRx()}_discoverPools(){let t=[];for(let e of Object.keys(this._hass.states))e.startsWith("sensor.sonos_pool_")&&t.push(e.replace("sensor.sonos_pool_",""));t.sort(),this._pools=t}_discoverDanteRx(){let t=[];for(let[e,i]of Object.entries(this._hass.states))e.startsWith("select.")&&e.includes("_rx_")&&t.push({id:e,name:i.attributes.friendly_name||e});t.sort((t,e)=>t.name.localeCompare(e.name)),this._danteRxEntities=t}render(){return L`
      <div class="form">
        <div class="row">
          <label>Name</label>
          <input
