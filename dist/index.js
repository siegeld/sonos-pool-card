...
`;class tl extends ts{static{this.styles=ta}setConfig(t){if(!t.pool)throw Error("pool is required");if(!t.zone_id)throw Error("zone_id is required");this._config=t}set hass(t){this._hass=t;let e=`sensor.sonos_pool_${this._config.pool}`,i=t.states[e];if(!i){this._player=null;return}let o=i.attributes.assignments||{};this._poolTotal=i.attributes.total||0,this._poolUsed=Object.keys(o).length;let s=o[this._config.zone_id];if(!s){this._player=null;return}let r=t.states[s];if(!r){this._player=null;return}let n=r.attributes;this._player={entityId:s,friendlyName:n.friendly_name||s.replace("media_player.",""),state:r.state,title:n.media_title||"",artist:n.media_artist||"",picture:n.entity_picture||"",volume:n.volume_level??0}}async _grab(){let t=await this._hass.connection.sendMessagePromise({type:"call_service",domain:"sonos_pool",service:"grab",service_data:{pool:this._config.pool,zone_id:this._config.zone_id},return_response:!0}),e=t?.response;if(!e?.dante_tx_l||!e?.dante_tx_r)return;let{dante_rx_l:i,dante_rx_r:o}=this._config;i&&this._hass.callService("select","select_option",{entity_id:i,option:e.dante_tx_l}),o&&this._hass.callService("select","select_option",{entity_id:o,option:e.dante_tx_r})}_release(){this._hass.callService("sonos_pool","release",{pool:this._config.pool,zone_id:this._config.zone_id})}_mediaCommand(t){this._player&&this._hass.callService("media_player",t,{entity_id:this._player.entityId})}_setVolume(t){if(!this._player)return;let e=parseFloat(t.target.value);this._hass.callService("media_player","volume_set",{entity_id:this._player.entityId,volume_level:e})}render(){return"compact"===this._config.mode?this._renderCompact():this._renderNormal()}_poolBadge(){return I`<span class="pool-count">${this._poolUsed}/${this._poolTotal}</span>`}_renderNormal(){let t=this._config.name||this._config.zone_id;if(!this._player)return I`
    ...
    `}_renderCompact(){if(!this._player)return I`
      ...
      <div class="compact-wrap">
        ...
        <div class="compact-info">
          ${t.title?I`<span class="compact-title">${t.title}</span>`:I`<span class="compact-title">${t.friendlyName}</span>`}
          ${t.artist?I`<span class="compact-artist">${t.artist}</span>`:V}
        </div>
